...
def test_ping_parser_success_no_packet_loss(mock_ping_command, localhost_address):
    ...
    assert result.hostname == localhost_address, f"{result.hostname=} == {localhost_address}"
...
def test_ping_parser_success_multiple_packets(mock_ping_command, icmp_count):
    ...
    assert result.packets_received == icmp_count, f"{result.packets_received=} == {icmp_count}"
    assert result.packet_loss == 0, f"{result.packet_loss=} == 0"
    assert result.rtt_min == RTT_MIN, f"{result.rtt_min=} == {RTT_MIN}"
    assert result.rtt_avg == RTT_AVG, f"{result.rtt_avg=} == {RTT_AVG}"
    assert result.rtt_max == RTT_MAX, f"{result.rtt_max=} == {RTT_MAX}"
...
def test_ping_parser_timeout(mock_ping_command, test_net_address):
    ...
    assert result.hostname == test_net_address, f"{result.hostname=} == {test_net_address}"
    assert result.ip_address == test_net_address, f"{result.ip_address=} == {test_net_address}"
    assert result.packets_sent == 1, f"{result.packets_sent=} == 1"
    ...
    assert result.error == PingStatus.TIMEOUT, f"{result.error=} == {PingStatus.TIMEOUT}"
...
def test_ping_parser_unknown_host(mock_ping_command):
    ...
    if platform.system() != "Windows":
        assert result.rtt_std_dev is None, f"{result.rtt_std_dev=} == None"

    assert result.error == PingStatus.UNKNOWN_HOST, f"{result.error=} == {PingStatus.UNKNOWN_HOST}"
...
def test_ping_parser_success_partial_loss(mock_ping_command, icmp_count):
    ...
    assert result.hostname == PARTIAL_LOSS_HOST, f"{result.hostname=} == {PARTIAL_LOSS_HOST}"
    assert result.ip_address == PARTIAL_LOSS_IP, f"{result.ip_address=} == {PARTIAL_LOSS_IP}"
    assert result.packets_sent == icmp_count, f"{result.packets_sent=} == {icmp_count}"
    assert result.packets_received == received_packets, f"{result.packets_received=} == {received_packets}"
    assert result.packet_loss == loss_percent, f"{result.packet_loss=} == {loss_percent}"
    assert result.rtt_min == RTT_MIN, f"{result.rtt_min=} == {RTT_MIN}"
    assert result.rtt_avg == RTT_AVG, f"{result.rtt_avg=} == {RTT_AVG}"
    assert result.rtt_max == RTT_STD_DEV, f"{result.rtt_max=} == {RTT_STD_DEV}"

    if platform.system() != "Windows":
        assert result.rtt_std_dev is not None, f"{result.rtt_std_dev=} == not None"
...
def test_ip_resolution_behavior(mock_ping_command):
    ...
    assert host_result.ip_address == ip_result.ip_address == LOCALHOST_IP, f"{host_result.ip_address=} == {ip_result.ip_address=} == {LOCALHOST_IP}"
...
def test_ping_parser_malformed_output(monkeypatch):
    ...
    assert result.rtt_max == 0, f"{result.rtt_max=} == 0"
    assert result.rtt_std_dev == 0, f"{result.rtt_std_dev=} == 0"
    assert result.error == PingStatus.PARSE_ERROR, f"{result.error=} == {PingStatus.PARSE_ERROR}"
...
def test_ping_command_execution_error(monkeypatch):
    ...
    assert result.rtt_max == 0, f"{result.rtt_max=} == 0"
    assert result.rtt_std_dev == 0, f"{result.rtt_std_dev=} == 0"
    assert result.error == PingStatus.EXECUTION_ERROR, f"{result.error=} == {PingStatus.EXECUTION_ERROR}"